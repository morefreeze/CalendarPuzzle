...
> 透明方块设置 + 胜利后换难度入口 + 分享卡灰度棋盘修复。

### Added
- **透明方块设置**：游戏内左下 ☰ 暂停菜单新增 🪟 透明方块 toggle。开启后落在日期 / 月份 / 星期格上的方块以 alpha=0.6 半透明叠加，下方数字与装饰透出，便于推断未对齐位置。本地持久化 storage key `calendarPuzzleTransparentBlocksOn`，默认关闭。中途可切换，下一帧立即生效。
- **胜利弹窗 "换个难度" 入口**：通关结算页新增第二个按钮（位于 "🎲 随机下一题" 下方），回到难度选择页让玩家直接挑下一档；同 `onBack` 路径，先 flush 存档再清 timer。结算卡 cardH 由 310 → 360 容纳 4 个按钮。
- **分享链接携带硬核标志**：sender 在硬核局通关后点 🎯 邀请朋友挑战，分享 query 追加 `hc=1`，标题前缀 "🔥 硬核 "；receiver 打开链接直接进入硬核局（同题面 + 同模式 = 可比挑战）。普通局分享行为不变。`tryLaunchShared` 解析 `hc=1` 后通过 `modeOpts` 透传给 `launchGameScene`。

### Fixed
- **分享卡灰度棋盘失效**（回归修复）：从 0.7.0 起，胜利分享的卡片缩略图无论从弹窗内 🎯 邀请朋友挑战还是微信胶囊菜单 ••• → 转发 / 朋友圈 都会带出原方块彩色，泄露解法。
  - 根因：原方案在 `wx.shareAppMessage` 调用前同步把主画布刷成灰度，但 (1) 微信不同机型对画布抓取时机不一致，胶囊菜单 `wx.onShareAppMessage` 回调里同步重绘并不保证生效；(2) 即便抓到也是带胜利弹窗 + 遮罩的画面，棋盘被遮住。
  - 修复：胜利时通过 `wx.createOffscreenCanvas` 渲一帧"灰度棋盘 + 无弹窗 + 无礼花"到独立离屏画布，`wx.canvasToTempFilePath` 异步落盘，PNG 路径写入 `shareState.imageUrl`，由 `buildShareData` 注入 `shareData.imageUrl`。两条分享路径（弹窗内按钮 / 胶囊菜单回调）都使用预生成的灰度图，主画布完全不受干扰，胜利弹窗 + 礼花同帧弹出无闪烁。

### Changed
- `shareState` 新增 `imageUrl` 字段 + `setImageUrl` / `clearImageUrl`；`setCurrent`（开新关）自动清旧路径。
- `game.js` 把主画布与 DPR 挂到 `GameGlobal.mainCanvas` / `GameGlobal.dpr`，供深层模块访问。

### Manual verification required (真机)
- 暂停菜单透明方块 toggle：默认关；开启后日期/月份/星期格上的方块半透明、数字可见；非今日格 + 今日金色格行为正确；关闭重启小游戏仍保持原 ON/OFF。
- 胜利弹窗 4 按钮顺序：🎲 随机下一题 / 换个难度 / 📤 分享朋友圈 / 🎯 邀请朋友挑战；点"换个难度"正确回到难度选择页。
- 胜利后 ~200ms 内点 🎯 邀请朋友挑战 → 分享卡片缩略图为干净灰度棋盘（无弹窗、无彩色）；右上角 ••• → 转发 / 📤 分享朋友圈 同样为灰度。

### Known limitations
- 胜利后约 100ms 内立刻点分享，imageUrl 可能尚未写入，会回退到微信实时截屏（彩色 + 弹窗）。真实玩家几乎不会这么快点击，可接受。
- 极老版本基础库不支持 `wx.createOffscreenCanvas`，回退到微信实时截屏（与本次修复前行为一致，无回归）。
