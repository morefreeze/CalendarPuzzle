...
| 17 | 设置含**键鼠按键重映射**（鼠标 + 键盘动作可改）；手柄不本地实现，依赖 Steam Input 社区配置 | 用户补充 A=a |
| 18 | **3 个预置皮肤**（Default / Pastel / Mono Focus），每个 = 棋盘配色 + 10 方块配色（+ 可选粒子）；引擎设计为 SkinResource 插槽，Phase 2 DLC 加主题皮肤包 | 用户补充 B=b |
...
│       │   └── skin_manager.gd          # 皮肤切换 + 加载 + 应用
│       ├── skins/                       # 3 预置皮肤；Phase 2 DLC 在此追加 .tres
│       │   ├── skin_resource.gd         # SkinResource 类定义
│       │   ├── default.tres             # 默认 10 色方案（鲜艳）
│       │   ├── pastel.tres              # 柔和低饱和度
│       │   └── mono_focus.tres          # 单色灰阶 + 单 accent
│       └── assets/                      # 该游戏专属（图标 / 音效 / 字体覆盖）
...
| Steam 成就 | 见下 | 新增 |
| 设置面板 | 音量 / 全屏 / 主题 / 语言 / **键鼠按键重映射** / 皮肤选择 / 重置 | 新增 |
| 皮肤系统 | 3 预置（Default / Pastel / Mono Focus）；设置中切换；Phase 2 DLC 加包 | 新增 |
...
├── profile.tres                         # 玩家档案
│   ├── settings: SettingsResource       # 音量 / 全屏 / 主题 / 语言 / kbm_bindings / current_skin_id
...
- **鼠键**：左键 = pointer，R/F = action（**默认绑定，玩家可在设置中改**）
...
- 玩家在设置面板 → "操作" 标签可重绑 5 个动作：`rotate` / `mirror` / `hint` / `undo` / `menu`
- 可绑定目标：单键 / 鼠标按键 / 修饰键组合（如 `Ctrl+Z`）
- UI 控件：`shared/ui/key_capture.gd`（标准 indie 风格——点击 → 显示"按下按键..."→ 捕获下一个 `InputEvent` → 检测冲突 → 保存）
- 存储：`SettingsResource.kbm_bindings: Dictionary[String, String]`（action 名 → 序列化的 InputEventKey）
- 默认值：`{"rotate": "R", "mirror": "F", "hint": "H", "undo": "Ctrl+Z", "menu": "Escape"}`
- 重置按钮：还原全部默认绑定
- **手柄不本地重映射**：Steam Input 平台原生支持每用户自定义手柄配置 + 社区分享配置，重复造轮子无意义。游戏只声明 5 个抽象 action，Steam Input 配置层负责映射到具体按键。
...
**组件库**（`shared/ui/`，~16 个）：Button / IconButton / Card / Modal / Toast / Slider / Switch / TabBar / Tooltip / ProgressBar / NumberStepper / DropdownMenu / Calendar（日历选题用）/ Thumbnail / Avatar / **KeyCapture**（按键重映射控件）。

每个组件配套 `shared/ui/examples/<name>_example.tscn` 与最小单元测试。

### 皮肤系统

**SkinResource 结构**（`games/calendar_puzzle/skins/skin_resource.gd`）：

```gdscript
class_name SkinResource extends Resource

@export var id: String                          # 唯一标识，例 "default" / "pastel"
@export var display_name: String                # i18n key，UI 显示
@export var board_palette: BoardPalette         # 棋盘背景 / 网格线 / 日期标记色
@export var block_colors: Array[Color]          # 10 种方块颜色（顺序对应 I/L/J/S/Z/P/Y/N/T/U）
@export var block_symbols: Array[Texture2D]     # 10 个色盲友好符号（可选；null 时用默认几何）
@export var particle_preset: PackedScene        # 放置/胜利粒子（可选）
@export var thumbnail: Texture2D                # 设置面板预览图（128×128）
```
...
| ID | 风格定位 | 棋盘 | 方块特征 |
|---|---|---|---|
| `default` | 鲜艳 / 现代设计 | 中性灰白 + 强对比 | 当前 10 色高饱和度方案 |
| `pastel` | 柔和 / 治愈 / 长时游玩 | 米白 + 淡蓝灰格 | 莫兰迪低饱和色调 |
| `mono_focus` | 极简 / 专注 / 失眠模式友好 | 纯黑/纯白 | 灰阶 + 单一 accent（强调当前选中块） |

**切换流程**：

```
设置 → 皮肤 → 点击预览缩略图 → SkinManager.apply(skin_id)
                                          ↓
                            SettingsResource.current_skin_id = skin_id
                            广播信号 → play_scene 重渲染棋盘 + 方块
                                          ↓
                            SaveAdapter.write 持久化
```

**Phase 2 DLC 钩子**：DLC 只需往 `games/calendar_puzzle/skins/` 投放新的 `.tres` 文件 + manifest 声明哪些 skin_id 属于该 DLC。`SkinManager` 启动时扫描整个 `skins/` 目录注册可用皮肤；DLC 未购买时对应 skin 在 UI 上灰显 + "购买 DLC 解锁" 入口。引擎本身不需改动。

**存储**：`SettingsResource.current_skin_id: String = "default"`。
...
| M4 | UI 外壳 + 存档 + 设置 | 3.5 | 70 | 主菜单 / 设置面板（含 KeyCapture 重映射 UI + 皮肤选择器）/ 3 槽手动存档 + 自动存档 + 缩略图 |
...
| M9 | 视觉打磨 + 音频 + 皮肤资产 | 3.5 | 70 | 组件库收尾；动画 / 缓动；10 SFX + 2 BGM 接入；色盲符号；亮/暗主题切换；**3 个 SkinResource 完成（Default / Pastel / Mono Focus）含缩略图** |
...
**累计 28-28.5 周 ≈ 6.7-7.2 个月** + **0.8-1.3 个月 buffer** = **8 个月**。新增的键位重映射 UI + 3 皮肤资产把 buffer 吃掉约 1 周；buffer 仍在可接受范围内，但已经偏紧——M9 之后若超期，**第一刀砍皮肤减到 2 个**（Default + Pastel，去 Mono Focus）。
...
1. 砍 Mono Focus 皮肤（剩 Default + Pastel；引擎依然支持，后续 patch 补）
2. 砍繁体中文（剩简中 + 英文）
3. 砍宣传片（用 5 张精修截图 + GIF 上架）
4. 砍 macOS 首发（Win + Linux + Deck 先发，Mac 1-2 月后补丁）
5. 砍 2 个最难做的成就（保留核心 18 个）

**不砍**：核心玩法 / 5 难度 / Steam Cloud / Deck 适配（决定 Verified 徽章，影响曝光最大）/ KBM 重映射（Steam 玩家基本预期）/ 至少 1 个非 Default 皮肤（皮肤系统的存在感比皮肤数量重要，1 个 alternate 能让"皮肤" feature 立住）。
